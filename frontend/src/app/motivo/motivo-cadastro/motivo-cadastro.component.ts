...
export class MotivoCadastroComponent implements OnInit {
  ...
  public deletarMotivo(): void {
    this.service.deletar(this.motivoForm.value.id).subscribe(() => {
      this.mensagem.add({ severity: 'success', summary: MessagemUtils.TITULO_DELETADO_SUCESSO, detail: "Motivo " + MessagemUtils.MENSAGEM_DADOS_DELETADOS });
      this.onCancel.emit();
      this.novaBusca.emit();
    })
  }
